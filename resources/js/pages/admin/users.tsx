 ...
 *  - handleSave/onSave prop removed — drawer handles its own router calls directly
 *  - Proper onSuccess / onError / onFinish callbacks in UserDrawer
 *  - Server-side validation errors shown inline per field
 *  - Delete modal with deleting state lives at the page level (not inside the modal)
 *  - QR and password reset buttons uncommented and wired up
...
const ROLE_CONFIG: Record<Role, { label: string; color: string; bg: string }> = {
  ...
  branch_manager: { label: "Branch Manager", color: "#9333ea", bg: "#f3e8ff" },
  servicer:       { label: "Servicer",        color: "#16a34a", bg: "#f0fdf4" },
};
...
function UserDrawer({
  ...
  const handleSave = () => {
    ...
    if (!isEdit && !form.password)      clientErrors.password = "Password is required for new users";
    if (needsBranch && !form.branch_id) clientErrors.branch_id = "Branch is required for this role";
    ...
    if (isEdit && user?.id) {
      router.put(route("admin.users.update", user.id), form, options);
    } else {
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(Object.entries(ROLE_CONFIG) as [Role, typeof ROLE_CONFIG[Role]][]).map(([role, cfg]) => (
                <button key={role} onClick={() => setForm((p) => ({ ...p, role, branch_id: ["super_admin", "admin"].includes(role) ? null : p.branch_id }))}
                  style={{
            ...
            {needsBranch && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                ...
                <select
                  ...
                  onChange={(e) => { setForm((p) => ({ ...p, branch_id: e.target.value ? Number(e.target.value) : null })); setErrors((p) => ({ ...p, branch_id: "" })); }}
                  style={{ ...inputStyle(!!errors.branch_id), cursor: "pointer" }}
                >
  ...
  return (
    <>
      ...
      <motion.div
        ...
        style={{ width: 400, background: "#ffffff", borderRadius: 20, overflow: "hidden", boxShadow: "0 20px 40px rgba(15,23,42,0.15)" }}
      >
        <div className="p-7">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "17px", fontWeight: 800, color: "#0f172a", marginBottom: "4px" }}>
                Delete User
              </h3>
          ...
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "13px", color: "#64748b", lineHeight: 1.5 }}>
        ...
        <div className="px-7 py-5 flex gap-3" style={{ borderTop: "1px solid #f1f5f9" }}>
          ...
          <button onClick={onConfirm} disabled={deleting}
            ...
            {deleting
              ? <><div style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(255,255,255,.4)", borderTopColor: "#fff", animation: "spin .7s linear infinite" }} /> Deleting…</>
              : "Delete User"}
          </button>
        </div>
      </motion.div>
...
export default function AdminUsers({ users, branches }: { users: User[]; branches: Branch[] }) {
  ...
  const handleGenerateQR = (user: User) => {
    router.post(route("admin.users.generate-qr", user.id), {}, {
      preserveScroll: true,
      onError: () => toast.error("Failed to generate QR token"),
    });
  };

  const handleRevokeQR = (user: User) => {
    router.post(route("admin.users.revoke-qr", user.id), {}, {
      preserveScroll: true,
      onError: () => toast.error("Failed to revoke QR token"),
    });
  };
  ...
  const handleDelete = () => {
    ...
    if (deleteTarget.last_active === "Now") {
      toast.error("Cannot delete a user with an active session.");
  ...
  return (
    <AdminLayout title="Users" active="users">
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            <AnimatePresence mode="popLayout">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: "48px", textAlign: "center", fontFamily: "'DM Sans',sans-serif", fontSize: "14px", color: "#94a3b8" }}>
                    No users found
                  </td>
                </tr>
              ) : filtered.map((user, i) => {
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{
                        ...
                        padding: "4px 10px", borderRadius: 100, background: roleConf.bg,
                        fontFamily: "'DM Sans',sans-serif", fontSize: "11px",
                        fontWeight: 600, color: roleConf.color,
                      }}>
                        <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: roleConf.color }} />
                        {roleConf.label}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <button
                        ...
                        <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "11px", fontWeight: 600, color: user.is_active ? "#16a34a" : "#dc2626" }}>
                    ...
                    {/* Last active */}
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "11px", color: user.last_active === "Now" ? "#16a34a" : "#94a3b8" }}>
                        {user.last_active === "Now"
                          ? <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block" /> Now</span>
                          : "Never"}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        <button onClick={() => setDrawer(user)} title="Edit user"
                          style={{
                            padding: "6px 12px", borderRadius: 8,
                            border: "1px solid #e2e8f0", background: "transparent",
                            fontFamily: "'DM Sans',sans-serif", fontSize: "12px",
                        ...
                        <button onClick={() => handleResetPassword(user)} title="Send password reset email"
                          style={{
                        ...
                        {user.role === "servicer" && (
                          <button
                            onClick={() => user.has_qr_token ? handleRevokeQR(user) : handleGenerateQR(user)}
                            title={user.has_qr_token ? "Revoke QR token" : "Generate QR token"}
                            style={{
                              padding: "6px 10px", borderRadius: 8,
                              border: `1px solid ${user.has_qr_token ? "#bbf7d0" : "#e2e8f0"}`,
                              background: user.has_qr_token ? "#f0fdf4" : "transparent",
                              fontSize: "12px",
                              color: user.has_qr_token ? "#16a34a" : "#64748b",
      ...
      <AnimatePresence>
        {drawer !== null && (
          <UserDrawer user={drawer} branches={branches} onClose={() => setDrawer(null)} />
        )}
      </AnimatePresence>
